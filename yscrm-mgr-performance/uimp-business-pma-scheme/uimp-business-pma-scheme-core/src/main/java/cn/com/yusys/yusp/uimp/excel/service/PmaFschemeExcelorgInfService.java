...
import cn.com.yusys.yusp.uimp.excel.repository.mapper.PmaFschemeExcelorgInfMapper;
import cn.com.yusys.yusp.uimp.excel.util.CommonExcelUtil;
import tk.mybatis.mapper.util.StringUtil;
/**
 * @项目名称: uimp-business-pma-scheme-core模块
 * @类名称: PmaFschemeExcelorgInfService
 * @类描述: #考核方案报表考核/归属机构信息表 服务类
 * @功能描述: 
 * @创建人: lixt1
 ...
 */
@Service
public class PmaFschemeExcelorgInfService extends CommonService {
	
	private static final Logger logger = LoggerFactory.getLogger(PmaFschemeExcelorgInfService.class);
	
	@Autowired
	private UserInfoService userInfo; 
	
    @Autowired
    private PmaFschemeExcelorgInfMapper pmaFschemeExcelorgInfMapper;

    @Override
    protected CommonMapper<?> getMapper() {
        return pmaFschemeExcelorgInfMapper;
    }
    ...
    public Integer deleteByTemplateId(String templateId) throws Exception {
    	...
    	try {
    		...
    		code = pmaFschemeExcelorgInfMapper.deleteByTemplateId(templateId);
    	} catch (Exception e) {
     ...
     */
    @Transactional(readOnly = false, rollbackFor = { Exception.class, RuntimeException.class })
    public Integer batchInsert(List<PmaFschemeExcelorgInf> dataList) throws Exception {
    	try {
    		if(dataList != null && dataList.size() > 0) {
		        ...
		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
		        	resultCount += pmaFschemeExcelorgInfMapper.batchInsert(dataList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
		        }
		        return resultCount;
    		} else {
	...
	public List<Map<String, Object>> getCellInfoByTemplateId(String templateId) {
		if(StringUtil.isNotEmpty(templateId)) {
    		return pmaFschemeExcelorgInfMapper.getCellInfoByTemplateId(templateId);
    	} else {
    ...
    /**
     * @方法名称: getPreviewInfo
     * @方法描述: 根据模板ID、数据日期、考核对象编号、考核对象类型，查询-所属机构单元格预览数据
     * @参数与返回说明: 
     * @算法描述:
     */
    @Transactional(readOnly = true)
	public List<Map<String, Object>> getPreviewInfo(String templateId, String etlDate, String evlObjId, String templateType, String evlObjType) {
		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
			return pmaFschemeExcelorgInfMapper.getPreviewInfo(templateId, etlDate, evlObjId, templateType, evlObjType);
		} else {
     ...
     * @方法描述: 更新所属机构信息表(备份表)-数据
     * @参数与返回说明: 
     * @算法描述:
     */
    @Transactional(readOnly = false, rollbackFor = { Exception.class, RuntimeException.class })
    public void updateBackupTableData(String etlDate, String templateId, String templateType, String evlObjId,
    		List<Map<String, Object>> orgCellInfoList) throws Exception {
    	try {
    		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && 
    				CommonExcelUtil.verifyDateStrByFormat(etlDate, "yyyyMMdd")) {
				// 根据模板ID、数据日期、考核对象编号删除备份表中对应数据
				if("02".equals(templateType)) {	// 单元格类型考核方案
					pmaFschemeExcelorgInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
				} else {	// 横/纵类型考核方案
					pmaFschemeExcelorgInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, "");
				}
    			// 保存所属机构数据
    			if(orgCellInfoList != null && orgCellInfoList.size() > 0) {
        			// 分批次保存
        			int index = orgCellInfoList.size() / 1000;
    		        int remainder = orgCellInfoList.size() % 1000;
    		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
    		        	pmaFschemeExcelorgInfMapper.batchInsertBackupTableData(etlDate, orgCellInfoList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
    		        }
        		}
    		} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-所属机构单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     */
    @Transactional(readOnly = true)
	public List<Map<String, Object>> getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(String templateId, 
		...
		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelorgInfMapper.getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
    	} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-我的考核方案所属机构单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
    ...
    public List<Map<String, Object>> getMySchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    	...
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && StringUtil.isNotEmpty(evlObjId)) {
    		return pmaFschemeExcelorgInfMapper.getMySchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, templateType, evlObjType,
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-机构辖内员工对象的考核方案所属机构单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     * 1、根据 方案类型/考核对象类型，查询对应权限数据
     * 2、根据hideRows、hideCols，排除对应行/列数据
    ...
    public List<Map<String, Object>> getOrgStaffSchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    		String templateType, String evlObjType, String hideRows, String hideCols) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelorgInfMapper.getOrgStaffSchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, 
	...
	public void copySchemeInf(String templateId, String newTemplateId) throws Exception {
	    try {
	    	pmaFschemeExcelorgInfMapper.copySchemeInf(templateId, newTemplateId);
	    } catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
