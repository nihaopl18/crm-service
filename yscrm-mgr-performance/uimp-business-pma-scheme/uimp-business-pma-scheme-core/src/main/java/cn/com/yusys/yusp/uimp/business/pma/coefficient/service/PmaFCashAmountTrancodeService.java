...
import cn.com.yusys.yusp.uimp.business.pma.coefficient.repository.mapper.PmaFCashAmountTrancodeMapper;
/**
 * @项目名称: uimp-business-pma-scheme-core模块
 * @类名称: PmaFCashAmountTrancodeService
 * @类描述: #服务类
 * @功能描述: 
 * @创建人: Administrator
 ...
 */
@Service
public class PmaFCashAmountTrancodeService extends CommonService {
    private static final Logger logger = LoggerFactory.getLogger(PmaFCashAmountTrancode.class);
    @Autowired
    private PmaFCashAmountTrancodeMapper pmaFCashAmountTrancodeMapper;
    @Autowired
    private UserInfoService userInfoService;
	@Value("${info.file.local-disk-path:/pmswork}")
	private String localDiskPath;
    @Override
    protected CommonMapper<?> getMapper() {
        return pmaFCashAmountTrancodeMapper;
    }
	@Transactional(readOnly = true)
	public List<Map<String, Object>> queryList(QueryModel model) {
    	PageHelper.startPage(model.getPage(), model.getSize());
		List<Map<String, Object>> list = this.pmaFCashAmountTrancodeMapper.querylist(model);
	...
	public void export(QueryModel model, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			String fileName = "CashAmountTrancode" + FastDateFormat.getInstance("yyyyMMddHHmmssSSS").format(new Date()) + ".xlsx";
			try {
	...
	public SXSSFWorkbook exportExcel(QueryModel model) throws IOException {
		...
		//查询选择开始和结束时间之间的数据
		List<Map<String, Object>> datelist= this.pmaFCashAmountTrancodeMapper.querylist(model);
	...
	public List<String> processImport(String fileRelatePath, MultipartFile mnultiFile, String fileName) throws Exception {
		...
		try {
			...
			if (retMap.get("flag").equals("true")) {// 校验通过
				try {
					List<PmaFCashAmountTrancode> dataList = (List<PmaFCashAmountTrancode>) retMap.get("dataList");
					for (int i = 0; i < dataList.size(); i++) {
						// 先删除后插入，覆盖之前的补录数据
						pmaFCashAmountTrancodeMapper.deleteData(dataList.get(i));
						pmaFCashAmountTrancodeMapper.insertSelective(dataList.get(i));
					}
	...
	private List<String> validateBaseInfo(XSSFSheet sheet) {
		...
		} else if (title.indexOf("业务量交易类型维护")<0) {	// 校验是否为标准模板
	...
	public Map<String, Object> getDataListFromSheet(List<String> errMsgList, XSSFSheet sheet) {
		...
		//用来存放数据的list
		List<PmaFCashAmountTrancode> dataList = new ArrayList<PmaFCashAmountTrancode>();
		// 获得总行数,该方法是获取的最后行数下标，所以实际数量应该+1
        int totalRowCount = sheet.getLastRowNum() + 1;
        XSSFRow row = null;
		for (int i = 2; i < totalRowCount; i++) {
	        row = sheet.getRow(i); 
    		PmaFCashAmountTrancode pmaFCashAmountTrancode = new PmaFCashAmountTrancode();
        	if (row.getCell(0) == null||"".equals(row.getCell(0).toString())) {
				errMsgList.add("第【" + (i + 1) + "】行,第【1】列：该单元格无数据，请查验。");
				continue;
			} else {
				int a = row.getCell(0).getCellType();
				if(SXSSFCell.CELL_TYPE_NUMERIC == row.getCell(0).getCellType()) {
					pmaFCashAmountTrancode.setTrancode(row.getCell(2).toString().substring(0, row.getCell(0).toString().length()-2));
				}else {
					if(row.getCell(0).toString().indexOf(".0")<0) {
						pmaFCashAmountTrancode.setTrancode(row.getCell(0).toString());
					}else {
						pmaFCashAmountTrancode.setTrancode(row.getCell(0).toString().substring(0, row.getCell(0).toString().length()-2));
					}
        	...
        	if (row.getCell(1) == null||"".equals(row.getCell(1).toString())) {
				...
				pmaFCashAmountTrancode.setTranname(row.getCell(1).toString());
			}
        	
        	dataList.add(pmaFCashAmountTrancode);
		}
		if(errMsgList.size()>0) {
	...
	public ResultDto<Integer> del(String ids) {
    	...
    	for (int i =0 ;i < id.length ; i++) {	
    		pmaFCashAmountTrancodeMapper.deleteByIds(id[i]);
    	}
    	result.setCode(0);
	...
	@Transactional(readOnly = false, rollbackFor = { Exception.class, RuntimeException.class })
	public ResultDto<PmaFCashAmountTrancode> add(PmaFCashAmountTrancode pmaFCashAmountTrancode) {
    	ResultDto<PmaFCashAmountTrancode> result = new ResultDto<PmaFCashAmountTrancode>();
		QueryModel modelNew = new QueryModel();
		modelNew.getCondition().put("trancode", pmaFCashAmountTrancode.getTrancode());
		String count = pmaFCashAmountTrancodeMapper.selectCount(modelNew);
		if(count.equals("0")) {
			pmaFCashAmountTrancodeMapper.insertSelective(pmaFCashAmountTrancode);
			result.setData(pmaFCashAmountTrancode);
	...
	@Transactional(readOnly = false, rollbackFor = { Exception.class, RuntimeException.class })
	public ResultDto<PmaFCashAmountTrancode> edit(PmaFCashAmountTrancode pmaFCashAmountTrancode) {
		ResultDto<PmaFCashAmountTrancode> result = new ResultDto<PmaFCashAmountTrancode>();
		pmaFCashAmountTrancodeMapper.updateByPrimaryKey(pmaFCashAmountTrancode);
		result.setData(pmaFCashAmountTrancode);
