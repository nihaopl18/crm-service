 ...
 * @类名称: PmaFschemeExcelorgInfMapper
 * @类描述: #考核方案报表考核/归属机构信息表 Dao类
 * @功能描述: 
 * @创建人: lixt1
 ...
 */
public interface PmaFschemeExcelorgInfMapper extends CommonMapper<PmaFschemeExcelorgInf> {
     ...
     * @算法描述:
     */
	Integer batchInsert(@Param("dataList") List<PmaFschemeExcelorgInf> dataList);
	...
	List<Map<String, Object>> getPreviewInfo(@Param("templateId") String templateId,@Param("etlDate") String etlDate, 
			...
			@Param("evlObjType") String evlObjType);
	
	/**
     * @函数名称:deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId
     * @函数描述:根据模板ID、数据日期、考核对象编号，删除所属机构信息表(备份表)数据
     * @参数与返回说明:
     * @算法描述:
     */
