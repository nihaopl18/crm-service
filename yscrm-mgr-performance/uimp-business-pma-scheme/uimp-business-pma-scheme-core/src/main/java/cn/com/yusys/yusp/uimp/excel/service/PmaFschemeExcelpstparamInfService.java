...
import cn.com.yusys.yusp.uimp.excel.repository.mapper.PmaFschemeExcelpstparamInfMapper;
import cn.com.yusys.yusp.uimp.excel.util.CommonExcelUtil;
import tk.mybatis.mapper.util.StringUtil;
/**
 * @项目名称: uimp-business-pma-scheme-core模块
 * @类名称: PmaFschemeExcelpstparamInfService
 * @类描述: #考核方案报表岗位参数信息表 服务类
 * @功能描述: 
 * @创建人: lixt1
 * @创建时间: 2020-05-19 12:38:06
 * @修改备注: 
 * @修改记录: 修改时间    修改人员    修改原因
 * -------------------------------------------------------------
 * @version 1.0.0
 ...
 */
@Service
public class PmaFschemeExcelpstparamInfService extends CommonService {
	
	private static final Logger logger = LoggerFactory.getLogger(PmaFschemeExcelpstparamInfService.class);
	
    @Autowired
    private PmaFschemeExcelpstparamInfMapper pmaFschemeExcelpstparamInfMapper;
    ...
    protected CommonMapper<?> getMapper() {
        return pmaFschemeExcelpstparamInfMapper;
    }
    ...
    public Integer deleteByTemplateId(String templateId) throws Exception {
    	...
    	try {
    		...
    		code = pmaFschemeExcelpstparamInfMapper.deleteByTemplateId(templateId);
    	} catch (Exception e) {
     ...
     */
    @Transactional(readOnly = false, rollbackFor = { Exception.class, RuntimeException.class })
    public Integer batchInsert(List<PmaFschemeExcelpstparamInf> dataList) throws Exception {
    	try {
    		if(dataList != null && dataList.size() > 0) {
		        ...
		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
		        	resultCount += pmaFschemeExcelpstparamInfMapper.batchInsert(dataList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
		        }
		        return resultCount;
    		} else {
	...
	public List<Map<String, Object>> getCellInfoByTemplateId(String templateId) {
    	if(StringUtil.isNotEmpty(templateId)) {
    		return pmaFschemeExcelpstparamInfMapper.getCellInfoByTemplateId(templateId);
    	} else {
    ...
    /**
     * @方法名称: getPreviewInfo
     * @方法描述: 根据模板ID、考核对象类型、考核对象编号，查询-岗位参数单元格预览数据
     * @参数与返回说明: 
     * @算法描述:
     */
    @Transactional(readOnly = true)
	public List<Map<String, Object>> getPreviewInfo(String templateId, String evlObjType, String evlObjId) {
		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(evlObjType)) {
			return pmaFschemeExcelpstparamInfMapper.getPreviewInfo(templateId, evlObjId);
		} else {
			logger.warn("templateId or evlObjType is null, can not getPreviewInfo");
			return null;
		}
	}
    	...
    	try {
    		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && 
				...
				if("02".equals(templateType)) {	// 单元格类型考核方案
					pmaFschemeExcelpstparamInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
				} else {	// 横/纵类型考核方案
					pmaFschemeExcelpstparamInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, "");
				}
    			// 保存岗位参数数据
    			if(pstparamCellInfoList != null && pstparamCellInfoList.size() > 0) {
        			// 分批次保存
        			int index = pstparamCellInfoList.size() / 1000;
    		        int remainder = pstparamCellInfoList.size() % 1000;
    		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
    		        	pmaFschemeExcelpstparamInfMapper.batchInsertBackupTableData(etlDate, pstparamCellInfoList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
    		        }
        		}
    		} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-岗位参数单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     */
    @Transactional(readOnly = true)
    public List<Map<String, Object>> getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(String templateId, String etlDate, String evlObjId) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelpstparamInfMapper.getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
    	} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-我的考核方案岗位参数单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
    ...
    public List<Map<String, Object>> getMySchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    	...
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && StringUtil.isNotEmpty(evlObjId)) {
    		return pmaFschemeExcelpstparamInfMapper.getMySchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, templateType, evlObjType,
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-机构辖内员工对象的考核方案岗位参数单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     * 1、根据 方案类型/考核对象类型，查询对应权限数据
     * 2、根据hideRows、hideCols，排除对应行/列数据
    ...
    public List<Map<String, Object>> getOrgStaffSchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    		String templateType, String evlObjType, String hideRows, String hideCols) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelpstparamInfMapper.getOrgStaffSchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, 
	...
	public void copySchemeInf(String templateId, String newTemplateId) throws Exception {
	    try {
	    	pmaFschemeExcelpstparamInfMapper.copySchemeInf(templateId, newTemplateId);
	    } catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-APP员工考核方案岗位参数单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     * 1、根据 方案类型/考核对象类型，查询对应权限数据
     * 2、根据hideRows、hideCols，排除对应行/列数据
    ...
    public List<Map<String, Object>> getAppMySchemeResultInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    		String templateType, String evlObjType, String hideRows, String hideCols) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && StringUtil.isNotEmpty(evlObjId)) {
    		return pmaFschemeExcelpstparamInfMapper.getAppMySchemeResultInfoFromBackupTable(templateId, etlDate, evlObjId, templateType, evlObjType,
    				StringUtil.isEmpty(hideRows) ? null : hideRows.split("\\$"),
					StringUtil.isEmpty(hideCols) ? null : hideCols.split("\\$"));
    	} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-APP机构考核方案岗位参数单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     * 1、根据 方案类型/考核对象类型，查询对应权限数据
     * 2、根据hideRows、hideCols，排除对应行/列数据
    ...
    public List<Map<String, Object>> getAppOrgSchemeResultInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    		String templateType, String evlObjType, String hideRows, String hideCols) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && StringUtil.isNotEmpty(evlObjId)) {
    		return pmaFschemeExcelpstparamInfMapper.getAppOrgSchemeResultInfoFromBackupTable(templateId, etlDate, evlObjId, templateType, evlObjType,
    				StringUtil.isEmpty(hideRows) ? null : hideRows.split("\\$"),
					StringUtil.isEmpty(hideCols) ? null : hideCols.split("\\$"));
    	} else {
