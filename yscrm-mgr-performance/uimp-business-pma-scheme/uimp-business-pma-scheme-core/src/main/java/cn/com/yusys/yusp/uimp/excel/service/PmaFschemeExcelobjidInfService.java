...
import cn.com.yusys.yusp.uimp.excel.repository.mapper.PmaFschemeExcelobjidInfMapper;
import cn.com.yusys.yusp.uimp.excel.util.CommonExcelUtil;
import tk.mybatis.mapper.util.StringUtil;
/**
 * @项目名称: uimp-business-pma-scheme-core模块
 * @类名称: PmaFschemeExcelobjidInfService
 * @类描述: #考核方案报表考核对象编号信息表 服务类
 * @功能描述: 
 * @创建人: lixt1
 ...
 */
@Service
public class PmaFschemeExcelobjidInfService extends CommonService {
	
	private static final Logger logger = LoggerFactory.getLogger(PmaFschemeExcelobjidInfService.class);
	
	@Autowired
	private UserInfoService userInfo; 
	
    @Autowired
    private PmaFschemeExcelobjidInfMapper pmaFschemeExcelobjidInfMapper;

    @Override
    protected CommonMapper<?> getMapper() {
        return pmaFschemeExcelobjidInfMapper;
    }
    ...
    public Integer deleteByTemplateId(String templateId) throws Exception {
    	...
    	try {
    		...
    		code = pmaFschemeExcelobjidInfMapper.deleteByTemplateId(templateId);
    	} catch (Exception e) {
     ...
     */
    @Transactional(readOnly = false, rollbackFor = { Exception.class, RuntimeException.class })
    public Integer batchInsert(List<PmaFschemeExcelobjidInf> dataList) throws Exception {
    	try {
    		if(dataList != null && dataList.size() > 0) {
		        ...
		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
		        	resultCount += pmaFschemeExcelobjidInfMapper.batchInsert(dataList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
		        }
		        return resultCount;
    		} else {
	...
	public List<Map<String, Object>> getCellInfoByTemplateId(String templateId) {
		if(StringUtil.isNotEmpty(templateId)) {
    		return pmaFschemeExcelobjidInfMapper.getCellInfoByTemplateId(templateId);
    	} else {
    	...
    	try {
    		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && 
				...
				if("02".equals(templateType)) {	// 单元格类型考核方案
					pmaFschemeExcelobjidInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
				} else {	// 横/纵类型考核方案
					pmaFschemeExcelobjidInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, "");
				}
    			// 保存岗位数据
    			if(dutyCellInfoList != null && dutyCellInfoList.size() > 0) {
    		        ...
    		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
    		        	pmaFschemeExcelobjidInfMapper.batchInsertBackupTableData(etlDate, dutyCellInfoList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
    		        }
        		}
    		} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-考核对象编号单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     */
    @Transactional(readOnly = true)
	public List<Map<String, Object>> getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(String templateId, 
		...
		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelobjidInfMapper.getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
    	} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-我的考核方案考核对象编号单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
    ...
    public List<Map<String, Object>> getMySchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    	...
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && StringUtil.isNotEmpty(evlObjId)) {
    		return pmaFschemeExcelobjidInfMapper.getMySchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, templateType, evlObjType,
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-机构辖内员工对象的考核方案考核对象编号单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     * 1、根据 方案类型/考核对象类型，查询对应权限数据
     * 2、根据hideRows、hideCols，排除对应行/列数据
    ...
    public List<Map<String, Object>> getOrgStaffSchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    		String templateType, String evlObjType, String hideRows, String hideCols) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelobjidInfMapper.getOrgStaffSchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, 
	...
	public void copySchemeInf(String templateId, String newTemplateId) throws Exception {
	    try {
	    	pmaFschemeExcelobjidInfMapper.copySchemeInf(templateId, newTemplateId);
	    } catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
