...
import cn.com.yusys.yusp.uimp.excel.repository.mapper.PmaFschemeExcelsvwInfMapper;
import cn.com.yusys.yusp.uimp.excel.util.CommonExcelUtil;
import tk.mybatis.mapper.util.StringUtil;
/**
 * @项目名称: uimp-business-pma-scheme-core模块
 * @类名称: PmaFschemeExcelsvwInfService
 * @类描述: #考核方案报表得分/计价/权重信息表 服务类
 * @功能描述: 
 * @创建人: lixt1
 ...
 */
@Service
public class PmaFschemeExcelsvwInfService extends CommonService {
	
	private static final Logger logger = LoggerFactory.getLogger(PmaFschemeExcelsvwInfService.class);
	
    @Autowired
    private PmaFschemeExcelsvwInfMapper pmaFschemeExcelsvwInfMapper;
    ...
    protected CommonMapper<?> getMapper() {
        return pmaFschemeExcelsvwInfMapper;
    }
    ...
    public Integer deleteByTemplateId(String templateId) throws Exception {
    	...
    	try {
    		...
    		code = pmaFschemeExcelsvwInfMapper.deleteByTemplateId(templateId);
    	} catch (Exception e) {
     ...
     */
    @Transactional(readOnly = false, rollbackFor = { Exception.class, RuntimeException.class })
    public Integer batchInsert(List<PmaFschemeExcelsvwInf> dataList) throws Exception {
    	try {
    		if(dataList != null && dataList.size() > 0) {
		        ...
		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
		        	resultCount += pmaFschemeExcelsvwInfMapper.batchInsert(dataList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
		        }
		        return resultCount;
    		} else {
	...
	public List<Map<String, Object>> getCellInfoByTemplateId(String templateId) {
    	if(StringUtil.isNotEmpty(templateId)) {
    		return pmaFschemeExcelsvwInfMapper.getCellInfoByTemplateId(templateId);
    	} else {
    	...
    	try {
    		if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && 
				...
				if("02".equals(templateType)) {	// 单元格类型考核方案
					pmaFschemeExcelsvwInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
				} else {	// 横/纵类型考核方案
					pmaFschemeExcelsvwInfMapper.deleteBackupTableDataByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, "");
				}
    			// 保存派生指标数据
    			if(svwCellInfoList != null && svwCellInfoList.size() > 0) {
        			// 分批次保存
        			int index = svwCellInfoList.size() / 1000;
    		        int remainder = svwCellInfoList.size() % 1000;
    		        for(int i = 0; i < (remainder == 0 ? index : index + 1); ++i) {
    		        	pmaFschemeExcelsvwInfMapper.batchInsertBackupTableData(etlDate, svwCellInfoList.stream().skip(i*1000).limit(1000).collect(Collectors.toList()));
    		        }
        		}
    		} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-得分/计价/权重单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     */
    @Transactional(readOnly = true)
    public List<Map<String, Object>> getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(String templateId, String etlDate, String evlObjId) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelsvwInfMapper.getCellInfoFromBackupTableByTemplateIdAndEtlDateAndEvlObjId(templateId, etlDate, evlObjId);
    	} else {
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-我的考核方案得分/计价/权重单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
    ...
    public List<Map<String, Object>> getMySchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    	...
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate) && StringUtil.isNotEmpty(evlObjId)) {
    		return pmaFschemeExcelsvwInfMapper.getMySchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, templateType, evlObjType,
     ...
     * @方法描述: 根据模板ID、数据日期、考核对象编号，查询-机构辖内员工对象的考核方案得分/计价/权重单元格数据(备份表)
     * @参数与返回说明: 
     * @算法描述:
     * 1、根据 方案类型/考核对象类型，查询对应权限数据
     * 2、根据hideRows、hideCols，排除对应行/列数据
    ...
    public List<Map<String, Object>> getOrgStaffSchemeCellInfoFromBackupTable(String templateId, String etlDate, String evlObjId, 
    		String templateType, String evlObjType, String hideRows, String hideCols) {
    	if(StringUtil.isNotEmpty(templateId) && StringUtil.isNotEmpty(etlDate)) {
    		return pmaFschemeExcelsvwInfMapper.getOrgStaffSchemeCellInfoFromBackupTable(templateId, etlDate, evlObjId, 
    ...
    /**
	 * @函数名称: copySchemeInf
	 * @函数描述: 复制考核方案信息-考核方案报表得分/计价/权重信息表
	 * @参数与返回说明:
	...
	public void copySchemeInf(String templateId, String newTemplateId) throws Exception {
	    try {
	    	pmaFschemeExcelsvwInfMapper.copySchemeInf(templateId, newTemplateId);
	    } catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
