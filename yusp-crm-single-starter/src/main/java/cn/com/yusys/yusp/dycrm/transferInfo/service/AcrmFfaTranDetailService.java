...
import cn.com.yusys.yusp.dycrm.transferInfo.repository.mapper.AcrmFfaTranDetailMapper;
 ...
 */
@Service
public class AcrmFfaTranDetailService extends CommonService {
	@Autowired
	private AcrmFfaTranDetailMapper acrmFfaTranDetailMapper;

	@Override
	protected CommonMapper<?> getMapper() {
		return acrmFfaTranDetailMapper;
	}
		...
		if (condition.get("rangeDate") instanceof List) {
			...
			try {
				...
				if (startDate != null && mon.getTime() <= startDate.getTime()) {
					list = acrmFfaTranDetailMapper.queryMonth(queryModel);
				} else {
					list = acrmFfaTranDetailMapper.queryAll(queryModel);
				}
	...
	public void export(QueryModel model, HttpServletResponse response) throws IOException {
		...
		String fileName = URLEncoder.encode("客户理财账户交易查询数据导出", "UTF-8").replaceAll("\\+", "%20");
		response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".xlsx");
		String templateFileName = "templates" + File.separator + "tranferinfoFA_templat.xlsx";
	...
	private List<TransferInfoFA> excel(QueryModel model) {
		PageHelper.startPage(model.getPage(), model.getSize());
		List<TransferInfoFA> list = null;
		SimpleDateFormat s = new SimpleDateFormat("yyyy-MM-dd");
		...
		if (condition.get("rangeDate") instanceof List) {
			...
			try {
				...
				if (startDate != null && mon.getTime() <= startDate.getTime()) {
					list = acrmFfaTranDetailMapper.excelMonth(model);
				} else {
					list = acrmFfaTranDetailMapper.excelAll(model);
				}
			} catch (ParseException e1) {
				// TODO 自动生成的 catch 块
				e1.printStackTrace();
