...
import cn.com.yusys.yscrm.fiexdstatement.repository.mapper.OcrmFClDepositincomeMapper;
...
@Service
public class OcrmFClDepositincomeService extends CommonService {

    @Autowired
    private OcrmFClDepositincomeMapper ocrmFClDepositincomeMapper;

    @Override
    protected CommonMapper getMapper() {
        return ocrmFClDepositincomeMapper;
    }
    @Autowired
    ...
    public List<Map<String, Object>> getIncomeList(QueryModel queryModel) {
        ...
        List<Map<String, Object>> list = ocrmFClDepositincomeMapper.getIncomeList(map);
    ...
    public void export(HttpServletResponse response, QueryModel queryModel) throws IOException {
        ...
        String fileName = URLEncoder.encode("存款收入", "UTF-8").replaceAll("\\+", "%20");
        response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".xlsx");
        //List<Map<String, Object>> list = this.getIncomeList(queryModel);
        Map<String, Object> map = new HashMap<>();
        String dataDate=(String)queryModel.getCondition().get("dataDate"); //日期天数
        String targetId=(String)queryModel.getCondition().get("targetId"); //客户经理
        String aumAvgBalanceStart=(String)queryModel.getCondition().get("aumAvgBalanceStart"); //AUM月日均起始
        ...
        exportFile(response, "depositincome_template.xlsx", "存款收入", resultList,dataDate);
    }
    ...
    public String getDataAuth(String role,String roleType){
        ...
        if (customerManage.contains(role)){ //客户经理
            dataAuth = "and CUSTOMER_MGR_ID = #{userCode}";
        }else if (patrolLeader.contains(role)){ //团队长
            dataAuth="and CUSTOMER_MGR_ID in (SELECT user_id FROM OCRM_F_CM_TEAM_CUST_MANAGER cm INNER JOIN OCRM_F_CM_MKT_TEAM mt ON cm.MKT_TEAM_ID =mt.MKT_TEAM_ID AND mt.TEAM_LEADER_ID =#{userCode})";
        }else if (subbranch.contains(role)||branch.contains(role) ){ //分支行
            if (roleType != ""){
                dataAuth="and CUSTOMER_MGR_ID in (SELECT u.USER_ID FROM ADMIN_SM_USER u  " +
        ...
        }else if (headOffice.contains(role) || headOfficeB.contains(role)){
            if (roleType != ""){
                dataAuth="and CUSTOMER_MGR_ID in (SELECT u.USER_ID FROM ADMIN_SM_USER u  " +
                        "INNER JOIN ADMIN_SM_USER_ROLE_REL ur ON u.USER_ID =ur.USER_ID " +
                        "INNER JOIN ADMIN_SM_ROLE r ON ur.ROLE_ID =r.ROLE_ID AND r.role_type = #{roleType} " +
